...
import (
	realdomain "github.com/akshanshgusain/Hexagonal-Architecture/domain"
	"github.com/akshanshgusain/Hexagonal-Architecture/dto"
	"github.com/akshanshgusain/Hexagonal-Architecture/errs"
	"github.com/akshanshgusain/Hexagonal-Architecture/mocks/domain"
...
func Test_should_return_an_error_from_the_server_side_if_the_new_account_cannot_be_created(t *testing.T) {
	...
	mockRepo.EXPECT().SaveA(account).Return(nil, errs.NewUnexpectedError("Unexpected database error"))
...
func Test_should_return_new_account_response_when_a_new_account_is_saved_successfully(t *testing.T) {
	...
	mockRepo.EXPECT().SaveA(account).Return(&accountWithId, nil)
	// Act
